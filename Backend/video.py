...
def save_video(video_url: str, target: Path) -> Path:
    """
...
def combine_videos(
    ...
    final_clip.write_videofile(str(combined_video_path), threads=threads)
...
def generate_video(
    ...
    result.write_videofile(str(target), threads=threads or 2)
    return target
